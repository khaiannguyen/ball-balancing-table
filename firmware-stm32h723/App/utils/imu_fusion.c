...
void kalman1d_init(kalman1d_t *kf, float Q_angle, float Q_bias, float R_measure)
{
    ...
    /* Zero covariance is suitable for an unknown initial state.
     * The filter converges through continuous measurement updates.
     */
}
                       ...
                       float *out_rate_unbiased)
{
    ...
    /* Update using accelerometer measurement */
    ...
    if (out_rate_unbiased) {
        *out_rate_unbiased = newRate - kf->bias;
    }

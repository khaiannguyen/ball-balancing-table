...
def draw_lcd_panel(panel_w, panel_h, state_text, roll_d, pitch_d, ball_on,
    ...
    metric_card(left_x1, y, left_x1 + col_w, y + row_h,
                "ROLL (TABLE)", f"{roll_d:+.1f}", GREEN, "")
    metric_card(left_x1 + col_w + col_gap, y,
                left_x2, y + row_h,
                "PITCH (TABLE)", f"{pitch_d:+.1f}", GREEN, "")

    y += row_h + int(7 * s)
    metric_card(left_x1, y, left_x1 + col_w, y + row_h,
                "X (Ball)", f"{x_mm:+.0f} mm", CYAN, "")
    metric_card(left_x1 + col_w + col_gap, y,
                left_x2, y + row_h,
                "Y (Ball)", f"{y_mm:+.0f} mm", CYAN, "")

    y += row_h + int(7 * s)
    metric_card(left_x1, y, left_x1 + col_w, y + row_h,
                "VX (Ball)", f"{vx:+.0f} mm/s", WHITE, None)
    metric_card(left_x1 + col_w + col_gap, y,
                left_x2, y + row_h,
                "VY (Ball)", f"{vy:+.0f} mm/s", WHITE, None)
    ...
    status_card(
        ...
        left_x2, y + status_h,
        "CAM", "OK" if cam_ok else "OK",
        GREEN if cam_ok else GREEN
    )
    ...
    if diag_r > 15:
        ...
        # Scale rieng cho hinh vang ben duoi.
        # 1.0 = kich thuoc cu, 0.6 = nho con 60%.
        yellow_scale = diag_scale * 0.6

        def diag_mm_to_px(px_mm, py_mm):
            return (int(round(diag_cx + px_mm * diag_scale)),
                    int(round(diag_cy - py_mm * diag_scale)))

        def yellow_mm_to_px(px_mm, py_mm):
            return (int(round(diag_cx + px_mm * yellow_scale)),
                    int(round(diag_cy - py_mm * yellow_scale)))
        ...
        if polyline_mm is not None:
            pts_px = np.array(
                [yellow_mm_to_px(px, py) for px, py in polyline_mm],
                dtype=np.int32)
            cv2.polylines(panel, [pts_px], False, YELLOW,
                          max(2, int(2 * s)), cv2.LINE_AA)

        elif phase_label == "CIRCLE":
            center_px = yellow_mm_to_px(0.0, 0.0)
            radius_px = int(round(K_CIRCLE_RADIUS * yellow_scale))
            ...
            pts_px = np.array(
                [yellow_mm_to_px(px, py) for px, py in pts_mm],
                dtype=np.int32)
        ...
        if ball_mm is not None:
            bx, by = yellow_mm_to_px(ball_mm[0], ball_mm[1])
        ...
        # Target/setpoint
        tx, ty = yellow_mm_to_px(x_mm, y_mm)

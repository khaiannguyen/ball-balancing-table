...
def draw_lcd_panel(panel_w, panel_h, state_text, roll_d, pitch_d, ball_on,
    ...
    cv2.putText(panel, "MODE 0", (panel_w - int(112 * s), int(20 * s)), font,
                fscale * 0.95, PANEL_TITLE_COLOR, fthick, cv2.LINE_AA)
    cv2.putText(panel, "SEL", (panel_w - int(70 * s), int(38 * s)), font,
                fscale * 0.95, PANEL_TITLE_COLOR, fthick, cv2.LINE_AA)
    y += int(line_h * 0.75)
    ...
    # ---- So do quy dao VANG DONG (giong het kieu ve tren video chinh):
    # duong hinh (tam giac/luc giac/tron/so-8) mau vang + cham target do +
    # bong trang (tam la cham xanh duong). KHONG con hang nut o duoi nua
    # (da bo theo yeu cau) nen so do duoc PHONG TO chiem gan het chieu
    # cao panel, giong ty le hinh tron/tam giac trong video chinh & anh
    # man LCD that nguoi dung gui. ----
    top_reserved = int(48 * s)      # tru cho "MODE 0 / SEL"
    bottom_margin = int(14 * s)
    diag_avail_h = panel_h - top_reserved - bottom_margin
    right_area_w = panel_w - (x0 + label_col_w + int(30 * s))
    diag_r = int(max(15, min(right_area_w * 0.48, diag_avail_h * 0.48)))

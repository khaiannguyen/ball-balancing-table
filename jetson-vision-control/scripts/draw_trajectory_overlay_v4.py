...
def draw_lcd_panel(panel_w, panel_h, state_text, roll_d, pitch_d, ball_on,
    ...
    """Panel LCD gia lap - giao dien toi gian, hien dai.

    Bo MODE/SEL. Ben trai la telemetry duoc chia thanh cac nhom:
    SYSTEM, ORIENTATION, BALL và SERVOS. Ben phai la ban do trajectory
    lon, giu cung mot scale mm -> pixel cho vong tron va tat ca hinh vang.
    """
    ...
    # -------------------------------------------------------------------------
    # Bố cục: trái = telemetry, phải = trajectory
    # -------------------------------------------------------------------------
    margin = int(12 * s)
    gap = int(10 * s)
    ...
    txt("BALANCE CONTROLLER", left_x1, int(42 * s), 0.30, MUTED, normal)
    ...
    chip_w = int(72 * s)
    chip_gap = int(7 * s)
    chip_x = left_x2 - chip_w * 3 - chip_gap * 2

    def chip(x, label, value, color):
        rr(x, chip_y1, x + chip_w, chip_y1 + chip_h,
           int(7 * s), CARD2, BORDER, 1)
        txt(label, x + int(7 * s), chip_y1 + int(10 * s),
            0.25, MUTED, thin)
        txt(value, x + int(7 * s), chip_y1 + int(21 * s),
            0.34, color, bold)

    chip(chip_x, "STATE", state_text[:8], GREEN)
    chip(chip_x + chip_w + chip_gap, "BALL",
         "ON" if ball_on else "OFF", GREEN if ball_on else RED)
    chip(chip_x + 2 * (chip_w + chip_gap), "CAM",
         "OK" if cam_ok else "ERR", CYAN if cam_ok else RED)
    ...
    y += row_h + gap
    metric_card(left_x1, y, left_x1 + col_w, y + row_h,
                "X", f"{x_mm:+.0f} mm", CYAN, "BALL POSITION")
    metric_card(left_x1 + col_w + col_gap, y,
                left_x2, y + row_h,
                "Y", f"{y_mm:+.0f} mm", CYAN, "BALL POSITION")

    y += row_h + gap
    metric_card(left_x1, y, left_x1 + col_w, y + row_h,
                "VX", f"{vx:+.0f}", WHITE, "mm/s")
    metric_card(left_x1 + col_w + col_gap, y,
                left_x2, y + row_h,
                "VY", f"{vy:+.0f}", WHITE, "mm/s")

    # Servo section
    y += row_h + gap
    txt("SERVO OUTPUT", left_x1, y + int(13 * s), 0.31, MUTED, bold)
    y += int(20 * s)
    ...
    for idx, value in enumerate((s1, s2, s3), start=1):
        ...
        centered(f"S{idx}", (sx1 + sx2) / 2, y + int(17 * s),
                 0.30, MUTED, thin)
        centered(f"{value:.0f}", (sx1 + sx2) / 2, y + int(40 * s),
                 0.56, BLUE, bold)
        centered("µs", (sx1 + sx2) / 2, y + int(51 * s),
                 0.24, MUTED, thin)

    # Instruction/footer
    footer_y = panel_h - int(19 * s)
    txt("HOLD RED TO STOP", left_x1, footer_y, 0.28, YELLOW, normal)
    phase_text = {
        "O_IDLE": "IDLE",
        ...
        "HEXAGON": "HEXAGON",
        "CIRCLE": "CIRCLE",
        "FIGURE8_H": "FIGURE-8 H",
        "FIGURE8_V": "FIGURE-8 V",
        "FIGURE8": "FIGURE-8",
    }.get(phase_label, "IDLE")
    ...
    txt("TRAJECTORY", right_x1 + int(14 * s), int(29 * s),
        0.38, WHITE, bold)
    txt(phase_text, right_x2 - int(125 * s), int(29 * s),
        0.32, YELLOW, normal)
    ...
    if diag_r > 15:
        # K_HEX_RADIUS is the physical radius represented by the red table
        # boundary. Every yellow trajectory uses exactly the same scale.
